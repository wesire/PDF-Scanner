...
from pdf_context_narrator.ocr import OCRMode
...
def ingest(
    ...
    workers: Optional[int] = typer.Option(
        None, "--workers", "-w", help="Number of parallel workers for multiprocessing"
    ),
    batch_size: Optional[int] = typer.Option(
        None, "--batch-size", "-b", help="Number of pages between checkpoints"
    ),
    memory_limit: Optional[int] = typer.Option(
        None, "--memory-limit", "-m", help="Memory limit in MB (requires psutil)"
    ),
    ...
    ocr_mode: OCRMode = typer.Option(
        OCRMode.AUTO,
        "--ocr-mode",
        help="OCR processing mode: off (no OCR), auto (OCR low-text pages), force (OCR all pages)",
    ),
) -> None:
    """
    ...
    if path.is_file():
        ...
        try:
            ...
            if result["completed"]:
                ...
                typer.echo("💾 Checkpoint saved. Run with --resume to continue.")
...
def search(
    ...
    data_dir: Optional[Path] = typer.Option(
        None, "--data-dir", "-d", help="Directory containing documents to search"
    ),
    keyword_weight: float = typer.Option(
        0.6, "--keyword-weight", help="Weight for keyword scoring (0-1)"
    ),
    vector_weight: float = typer.Option(
        0.4, "--vector-weight", help="Weight for vector scoring (0-1)"
    ),
    ...
    results = engine.search(
        query=query, top_k=limit, keyword_weight=keyword_weight, vector_weight=vector_weight
    )
...
def rebuild_index(
    ...
    index_path: Optional[Path] = typer.Option(
        None, "--index-path", "-i", help="Path to save index"
    ),
    model: str = typer.Option(
        "all-MiniLM-L6-v2", "--model", "-m", help="Sentence transformer model name"
    ),
) -> None:
    """
