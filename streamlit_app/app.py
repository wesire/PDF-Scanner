...
def render_upload_view():
    ...
    with st.container():
        ...
        with col1:
            st.checkbox("Force re-indexing", value=False)
        with col2:
...
def render_search_view():
    ...
    col1, col2, col3 = st.columns([1, 1, 2])
    with col1:
        st.selectbox("Search Type", ["Full Text", "Semantic", "Keyword"])
    with col2:
        st.selectbox("Sort By", ["Relevance", "Date", "Title"])
...
def render_narrative_view():
    ...
    with col1:
        narrative_type = st.selectbox(
            "Narrative Type", ["Chronological", "Thematic", "Analytical", "Summary"]
        )
    with col2:
            ...
            with col2:
                if st.button("📥 Export as JSON", use_container_width=True):
                    json_data = json.dumps(
                        {"narrative": narrative, "type": narrative_type}, indent=2
                    )
                    st.download_button(
                        "Download JSON",
                        json_data,
...
def render_settings_view():
    ...
    with col2:
        ...
        st.checkbox("Enable Structured Logging", value=False)

    st.subheader("Data Directories")
    st.text_input("Data Directory", value="data")
    st.text_input("Cache Directory", value="cache")
    st.text_input("Logs Directory", value="logs")
