...
class TestE2EWorkflow:
    ...
    def test_workflow_with_filters(self, tmp_path):
        ...
        result = runner.invoke(
            app,
            ["search", "query", "--format", "json", "--limit", "5"]
        )
        assert result.exit_code == 0
        ...
        result = runner.invoke(
            app,
            ["export", "json", str(output_file), "--filter", "status:processed"]
        )
        assert result.exit_code == 0
    ...
    def test_workflow_with_config_profiles(self, tmp_path):
        ...
        for profile in profiles:
            result = runner.invoke(
                app,
                ["--profile", profile, "ingest", str(pdf_file)]
            )
    ...
    def test_verbose_and_structured_logging(self, tmp_path):
        ...
        result = runner.invoke(
            app,
            ["--structured-logs", "search", "test query"]
        )
        assert result.exit_code == 0
